...
class CurtainClab {
	...
	_setLiHeight(options, maxInView, disabled) {
		if (
			...
			this.async(() => {
				...
				const listHeight =
					options.length > 0 ? `${this.maxHeight * options.length}px` : '0';
  ...
  ----------*/
	_compHighlight(highlighted, option) {
		return highlighted[this.valueField] === option[this.valueField]
			? 'selected'
			: '';
	}
  ...
  ----------*/
	scrollToHighlight(i, goesUp) {
		...
		const visible =
			offsetTop < scrollTop || offsetTop >= scrollTop + h ? false : true;
