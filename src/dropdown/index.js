...
class DropdownClab {
	...
	beforeRegister() {
		...
		this.observers = ['_triggerFetchOptions(url)', '_setOptions(optionsFn)'];
	}
  ...
  ----------*/
	...
	_compSelectedLabel(option, label) {
		return !isNil(option) && !isNilOrEmptyStr(label) ? option[label] : null;
	}
	...
	_compLabelClass(selectedLabel, placeholder) {
		return !isNilOrEmptyStr(selectedLabel) ? 'selected' : 'placeholder';
	}

	_compLabelVisible(selectedLabel, placeholder) {
		return !isNilOrEmptyStr(selectedLabel) ? selectedLabel : placeholder;
	}
	...
	_compToggleList(disabled, id) {
		return evt => {
			if (!disabled) {
				const isSpan = e =>
					e.target.localName === 'span' && e.path[4] && e.path[4].id === id;
				const isDiv = e =>
					e.target.localName === 'div' && e.path[3] && e.path[3].id === id;
				const isInput = e =>
	...
	_compSearchVisibleValue(searchValue, selectedLabel) {
		if (!isNilOrEmptyStr(selectedLabel)) {
			return isNil(searchValue) ? selectedLabel : searchValue;
		}
	...
	getValue() {
		...
		if (this.isNotValorized()) {
			...
			this.selected instanceof String
		) {
			v = this.selected;
			...
			console.error(
				this.is + ': Invalid value type [' + typeof this.selected + ']'
			);
		}
		return v;
	}
	...
	getValueObject() {
		...
		if (this.isNotValorized(this.selected)) {
			...
			if (v === undefined) {
				console.warn(
					this.is +
						': There is no option with value equal to [' +
						this.selected +
						']'
				);
			}
